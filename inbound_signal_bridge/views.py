...
from utils.Orchestrator.orchestrators import isb_orchestrator
...
def index(request):
    ...
    return render(request, 'inbound_signal_bridge.html', {'display_name': display_name})
...
def chat(request):
    ...
    try:
        user_id, session_id = setup_user_context(request, 'ISB')
    except _AuthError as e:
        return e.response

    history = get_history(user_id, 'ISB')
    with request_queue:
        result = isb_orchestrator.process_message(user_message, history)
    ...
    save_messages(user_id, 'ISB', user_message, response_text)
    ChatLog.objects.create(
        module='ISB', session_id=session_id, user_id=user_id,
...
def chat_stream(request):
    ...
    def event_stream():
        ...
        try:
            ...
            save_messages(user_id, 'ISB', user_message, full_response)
