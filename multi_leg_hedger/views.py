"""MLH views: chat (blocking + SSE streaming) for the Multi-Leg Hedger plugin."""
...
def index(request):
    ...
    return render(request, 'multi_leg_hedger.html', {'display_name': display_name})
...
def chat(request):
    ...
    try:
        user_id, session_id = setup_user_context(request, 'MLH')
    except _AuthError as e:
        return e.response

    history = get_history(user_id, 'MLH')
    with request_queue:
        result = mlh_orchestrator.process_message(user_message, history)
    ...
    save_messages(user_id, 'MLH', user_message, response_text)
    ChatLog.objects.create(
        module='MLH', session_id=session_id, user_id=user_id,
...
def chat_stream(request):
    ...
    def event_stream():
        ...
        runware_task_id = ""
        try:
            for event in mlh_orchestrator.stream_message(user_message, history):
                t = event.get('t')
                if t == 'chunk':
