...
def index(request):
    return render(request, 'multi_leg_hedger.html')
...
def chat(request):
    ...
    mlh_memory[session_id].append({"role": "user", "content": user_message})
    mlh_memory[session_id].append({"role": "assistant", "content": response_text})
...
def chat_stream(request):
    ...
    def event_stream():
        ...
        try:
            ...
            mlh_memory[session_id].append({"role": "user", "content": user_message})
            mlh_memory[session_id].append({"role": "assistant", "content": full_response})
            cost_usd = (
            ...
            try:
                ChatLog.objects.create(
                    module='MLH', session_id=session_id,
