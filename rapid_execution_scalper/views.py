...
from utils.Orchestrator.orchestrators import res_orchestrator
...
def index(request):
    ...
    return render(request, 'rapid_execution_scalper.html', {'display_name': display_name})
...
def chat(request):
    ...
    try:
        user_id, session_id = setup_user_context(request, 'RES')
    except _AuthError as e:
        return e.response

    history = get_history(user_id, 'RES')
    with request_queue:
        result = res_orchestrator.process_message(user_message, history)
    ...
    save_messages(user_id, 'RES', user_message, response_text)
    ChatLog.objects.create(
        module='RES', session_id=session_id, user_id=user_id,
